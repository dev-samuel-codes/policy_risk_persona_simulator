...
> [!IMPORTANT]
> 이 README는 `main` 브랜치의 2026-08-26 구현 상태와 2026년 오픈소스 개발자대회 결과보고서를 함께 기준으로 작성했습니다. 현재 제품은 예상 민원과 정책 사각지대를 **정성적으로 검토**하는 도구이며, 과거 프로토타입의 수치형 `risk_score`와 `risk_category`는 현재 API 결과에서 제거되었습니다.

> [!CAUTION]
> 대회 제출 프로젝트명에는 "정책 민원 예측 시뮬레이터"라는 표현이 포함되지만, 여기서 예측은 합성 시민의 반응 시나리오를 사전에 생성·검토한다는 의미입니다. 결과는 실제 여론, 민원 발생확률, 수급 자격, 법률 판단, 행정 처분 또는 특정 집단의 실제 의견을 의미하지 않습니다.
...
| 역할 | 모델 | 현재 동작 |
|---|---|---|
| 시민 민원 생성·정책 파일 필드 추출 | `Qwen/Qwen3-8B` | `main`의 기본 생성 모델 |
| 정책·민원 임베딩 검색 | `snunlp/KR-SBERT-V40K-klueNLI-augSTS` | 768차원 임베딩과 Chroma 검색 |

> [!NOTE]
> 대회 결과보고서에는 자원 부족 시 `Qwen/Qwen3-4B-Instruct-2507`로 대체하는 운영안이 기재되어 있습니다. 그러나 현재 `main`의 `QWEN_MODEL_NAME`은 `Qwen/Qwen3-8B`로 고정되어 있으며 자동 4B 폴백은 구현되어 있지 않습니다. 따라서 4B 모델은 현재 기능이 아니라 문서상 운영 대안입니다.
...
- 현재 `main`은 Qwen3-8B를 고정 사용하며 결과보고서의 4B 대체 운용안을 자동 적용하지 않습니다.
...
| 자산 | 결과보고서 기재 내용 | 조치 |
|---|---|---|
| `Qwen/Qwen3-8B` | Apache-2.0 | 모델 파일·고지 조건 확인 |
| `Qwen/Qwen3-4B-Instruct-2507` | Apache-2.0 | 현재 자동 폴백 미구현, 사용 시 별도 고지 |
